...
class DlCommand extends Command {
  async run() {
    ...
    const configJson = await fs.readJson(
      path.join(this.config.configDir, configFile)
    );
    ...
    this.dateFormat = configJson.dateFormat || "YYYY-MM-DD";
    this.postfix = configJson.postfixFileName;

    this.fileName = this.getFileNameToDownload(configJson.fileName, day);
  ...
  getFileNameToDownload(defaultFileName, day) {
    ...
    if (day) {
      ...
      // return `${this.prefix}.${year}-${month}-${day}.log${this.postfix}`
      return `${this.prefix}${this.getFormattedDate(day)}${this.postfix}`;
    }
...
DlCommand.args = [{ name: "first" }, { name: "second" }];
